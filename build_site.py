import re

with open('index.html', 'r', encoding='utf-8') as f:
    html = f.read()

# Extract CSS
css_match = re.search(r'<style>(.*?)</style>', html, re.DOTALL)
css = css_match.group(1) if css_match else ''
css += '''
/* --- INNER PAGE HEADER --- */
.page-header {
  padding: 120px 0 80px;
  background: linear-gradient(rgba(10,10,10,0.8), rgba(10,10,10,0.95)), url('https://images.unsplash.com/photo-1596755389378-c31d21fd1273?w=1920&q=80') center/cover;
  text-align: center;
  border-bottom: 2px solid var(--gold);
}
.page-header h1 {
  font-family: var(--hd);
  font-size: 3.5rem;
  color: var(--white);
  margin-bottom: 12px;
}
.page-header h1 span { color: var(--gold); font-style: italic; }
.breadcrumb {
  font-size: 0.8rem;
  color: var(--gold);
  text-transform: uppercase;
  letter-spacing: 2px;
  font-weight: 600;
}
.breadcrumb a { color: var(--white); transition: color 0.3s; }
.breadcrumb a:hover { color: var(--gold); }
.breadcrumb i { font-size: 0.6rem; margin: 0 8px; color: var(--gray); }
'''

with open('style.css', 'w', encoding='utf-8') as f:
    f.write(css)

# Extract JS
js_match = re.search(r'<script>\s*(AOS.*?)\s*</script>', html, re.DOTALL)
js = js_match.group(1) if js_match else ''
# Remove smooth scroll script because it's multi-page now
js = re.sub(r'// Smooth scroll.*?\}\);\}\);', '', js, flags=re.DOTALL)

with open('script.js', 'w', encoding='utf-8') as f:
    f.write(js)

# Prepare Base HTML by removing styles and inline scripts, linking to external
base_html = re.sub(r'<style>.*?</style>', '<link rel="stylesheet" href="style.css">', html, flags=re.DOTALL)
base_html = re.sub(r'<script>\s*AOS.*?</script>', '<script src="script.js"></script>', base_html, flags=re.DOTALL)

# Update Navigation Links
nav_map = {
    '#home': 'index.html',
    '#about': 'about.html',
    '#services': 'services.html',
    '#courses': 'courses.html',
    '#gallery': 'gallery.html',
    '#vlogs': 'vlogs.html',
    '#contact': 'contact.html'
}

for old, new in nav_map.items():
    base_html = base_html.replace(f'href="{old}"', f'href="{new}"')

# Remove 'active' class from index.html links so we can add it dynamically
base_html = base_html.replace('class="active"', '')

# Extract components
def extract_section(html_str, sec_id):
    pattern = f'(<!-- --- {sec_id} --- -->.*?)(?=<!-- ---)'
    match = re.search(pattern, html_str, re.DOTALL)
    if not match:
        pattern = f'(<!-- --- {sec_id} --- -->.*)'
        match = re.search(pattern, html_str, re.DOTALL)
    return match.group(1) if match else ''

head_top = base_html.split('<!-- --- HERO --- -->')[0]
footer_bottom = '<!-- --- FOOTER --- -->' + base_html.split('<!-- --- FOOTER --- -->')[1]

hero = extract_section(base_html, 'HERO')
services = extract_section(base_html, 'SERVICES')
about = extract_section(base_html, 'ABOUT \+ COURSES SPLIT')
why = extract_section(base_html, 'WHY CHOOSE US')
stats = extract_section(base_html, 'STATS BAR')
gallery = extract_section(base_html, 'GALLERY \+ TESTIMONIALS')
vlogs = extract_section(base_html, 'VLOGS')
cta = extract_section(base_html, 'CTA')
contact = extract_section(base_html, 'CONTACT')

# Create index.html (Home)
with open('index.html', 'w', encoding='utf-8') as f:
    f.write(base_html)

# Helper to generate page
def make_page(filename, title, span, content):
    header = f'''
<!-- --- PAGE HEADER --- -->
<section class="page-header">
  <div class="container" data-aos="fade-up">
    <h1>{title.split(' ')[0]} <span>{' '.join(title.split(' ')[1:])}</span></h1>
    <div class="breadcrumb">
      <a href="index.html">Home</a> <i class="fas fa-chevron-right"></i> {title}
    </div>
  </div>
</section>
'''
    # Set active class for this page
    page_head = head_top.replace(f'href="{filename}"', f'href="{filename}" class="active"')
    
    full_html = page_head + header + content + cta + footer_bottom
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(full_html)

# Generate inner pages
make_page('about.html', 'About Us', 'Us', about + why + stats)
make_page('services.html', 'Our Services', 'Services', services)
make_page('courses.html', 'Professional Courses', 'Courses', about + why) # Reusing split layout for courses
make_page('gallery.html', 'Our Gallery', 'Gallery', gallery)
make_page('vlogs.html', 'Latest Vlogs', 'Vlogs', vlogs)
make_page('contact.html', 'Contact Us', 'Us', contact)

print("Site built successfully!")
